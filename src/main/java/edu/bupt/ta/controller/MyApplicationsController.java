...
public class MyApplicationsController {
    ...
    private void refresh() {
        ...
        allApplications = services.applicationService().getApplicationsByApplicant(applicantId).stream()
                .filter(app -> app.getStatus() != ApplicationStatus.CANCELLED)
                // 过滤掉关联岗位为 DRAFT 的申请（岗位未发布，申请不应显示）
                .filter(app -> services.jobService().getJob(app.getJobId())
                        .map(job -> job.getStatus() != edu.bupt.ta.enums.JobStatus.DRAFT)
                        .orElse(false))
